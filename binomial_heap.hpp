...
/*
        bnode -
                child    - points to node with value such that child value >
   parent value sibling  - points to node with shared parent, creates Single
...
class BinomialHeap {
 ...
 protected:
   ...
   * 		The heap is appended to the end of the child list to maintain child
   * order of roots from min to max Assumes heapA and heapB siblings are ordered
   * by root order(sibling count)
   *
   ...
   * 		such that the either heapA becomes a child of heapB or the
   * reverse
   */
  static BinomialNode<T>* unionHeap(BinomialNode<T>* heapA,
                                    BinomialNode<T>* heapB);
  /* mergeHeap
   ...
   * 		Assumes heapA and heapB siblings are ordered by root order(sibling
   * count) Consolidating two heaps maintains merged ordering
   *
   * 	HeapA - root of heap to union with heapB
   * 	HeapB - root of heap to union with heapA
   *
   ...
   */
  static BinomialNode<T>* mergeHeap(BinomialNode<T>* heapA,
                                    BinomialNode<T>* heapB);
  /*
   * order
...
int BinomialNode<T>::size(int& i) {
  ++i;
  if (sibling) sibling->size(i);
  if (child) child->size(i);
...
T BinomialHeap<T>::extractMin() {
  BinomialNode<T>*prev = nullptr, *min = nullptr, *temp = nullptr,
  *next = nullptr;
  T minValue;
  if (head) {
...
void BinomialHeap<T>::decreaseKey(T v, T newV) {
  if (head) {
    if (v > newV) {
       ...
       */
      BinomialNode<T>*current = head->find(v), *parent = nullptr;
      T temp;
      if (current) {
        current->value = newV;
      ...
      throw BinomialHeap<T>::Exception(
...
template <typename T>
BinomialNode<T>* BinomialHeap<T>::unionHeap(BinomialNode<T>* heapA,
                                            BinomialNode<T>* heapB) {
  /*
  ...
  if (heapU) {
     ...
     *           |_|->    |_|->
     */
    BinomialNode<T>*cur = heapU,  // current node to append before or after
...
template <typename T>
BinomialNode<T>* BinomialHeap<T>::mergeHeap(BinomialNode<T>* heapA,
                                            BinomialNode<T>* heapB) {
  // modify to insert
  ...
  if (heapA || heapB) {
    if (heapA && !heapB) {
      ...
      BinomialNode<T>*temp = nullptr, *next = nullptr, *prev = nullptr,
      *cur = nullptr;
